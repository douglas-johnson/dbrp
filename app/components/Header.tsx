...
function HeaderMenuMobileToggle() {
	...
	return <button type="button" className="button is-transparent" onClick={() => useableNavContext?.menu?.current?.showModal()}>
		<svg className="size-square-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
...
function SearchToggle() {
	const useableNavContext = useContext( NavContext );
	return <button type="button" className="button is-transparent" onClick={() => useableNavContext?.search?.current?.showModal()}>
...
function CartBadge({count}: {count: number}) {
 ...
 return <button type="button" className="button is-transparent" onClick={() => useableNavContext?.cart?.current?.showModal()}>
